...
# path to the private key and certificates in the container, and the filenames
...
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to p12)
...
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to ca)
...
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to all)
# - sh.acme.autoload.certpath: The path to the certificate files within the container (default is /config)
# - sh.acme.autoload.keyname: The name of the key file to be created
# - sh.acme.autoload.certname: The name of the cert file to be created
# - sh.acme.autoload.caname: The name of the ca file to be created
...
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to cert)
# - sh.acme.autoload.certpath: The path to the certificate files within the container (default is /config)
# - sh.acme.autoload.keyname: The name of the key file to be created
# - sh.acme.autoload.certname: The name of the cert file to be created
#
...
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to fullchain)
# - sh.acme.autoload.certpath: The path to the certificate files within the container (default is /config)
# - sh.acme.autoload.keyname: The name of the key file to be created
# - sh.acme.autoload.fullchainname: The name of the fullchain file to be created
#
...
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to bundledchain)
...
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to bundledcert)
# - sh.acme.autoload.certpath: The path to the bundled file within the container (default is /config)
# - sh.acme.autoload.bundlename: The name of the bundled file to be created
#
# bundledca requires the following labels:
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to bundledca)
# - sh.acme.autoload.certpath: The path to the bundled file within the container (default is /config)
# - sh.acme.autoload.bundlename: The name of the bundled file to be created
#
# crashplan requires the following labels:
# - sh.acme.autoload.domain: The domain name of the certificate
# - sh.acme.autoload.certtype: The type of certificate files to be generated (set it to crashplan)
...
dockermulti_deploy() {
	...
	if ! container_name=$(docker inspect --format '{{.Name}}' "$container_id" | tr -d /); then
		_err "Error getting container name for $container_id"
		 ERROR_FLAG=true
            continue
        fi
	
	    _info "Processing container $container_id ($container_name)"
        ...
        if ! CERTPATH_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.certpath"}}' "$container_id"); then
            _err "Error getting sh.acme.autoload.certpath for $container_id ($container_name)"
            ERROR_FLAG=true
            continue
        fi
        if ! CERTTYPE_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.certtype"}}' "$container_id"); then
            _err "Error getting sh.acme.autoload.certtype for $container_id ($container_name)"
            ERROR_FLAG=true
            continue
        fi
        ...
        if [ "$CERTTYPE_LABEL" == "p12" ]; then
            ...
            if ! P12NAME_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.p12name"}}' "$container_id"); then
                _err "Error getting sh.acme.autoload.p12name for $container_id ($container_name)"
                ERROR_FLAG=true
                continue
            fi

            # Copy the _import_pkcs12 file into the container
            docker cp "$_import_pkcs12" "$container_id:$DESTINATION_PATH/$P12NAME_LABEL" || { _err "Error copying PKCS12 file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            _info "$CERTTYPE_LABEL certificate type files successfully copied to container: $container_id ($container_name)"
            ...
            if ! KEYNAME_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.keyname"}}' "$container_id"); then
                _err "Error getting sh.acme.autoload.keyname for $container_id ($container_name)"
                ERROR_FLAG=true
                continue
            fi
            if ! CERTNAME_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.certname"}}' "$container_id"); then
                _err "Error getting sh.acme.autoload.certname for $container_id ($container_name)"
                ERROR_FLAG=true
                continue
            fi
            if ! CANAME_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.caname"}}' "$container_id"); then
                _err "Error getting sh.acme.autoload.caname for $container_id ($container_name)"
                ERROR_FLAG=true
                continue
            fi

            # Copy the _ckey, _ccert, and _cca files into the container
            docker cp "$_ckey" "$container_id:$DESTINATION_PATH/$KEYNAME_LABEL" || { _err "Error copying key file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            docker cp "$_ccert" "$container_id:$DESTINATION_PATH/$CERTNAME_LABEL" || { _err "Error copying cert file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            docker cp "$_cca" "$container_id:$DESTINATION_PATH/$CANAME_LABEL" || { _err "Error copying ca file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            _info "$CERTTYPE_LABEL certificate type files successfully copied to container: $container_id ($container_name)"
            ...
            if ! FULLCHAINNAME_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.fullchainname"}}' "$container_id"); then
                _err "Error getting sh.acme.autoload.fullchainname for $container_id ($container_name)"
                ERROR_FLAG=true
                continue
            fi
            ...
            docker cp "$_ccert" "$container_id:$DESTINATION_PATH/$CERTNAME_LABEL" || { _err "Error copying cert file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            docker cp "$_cca" "$container_id:$DESTINATION_PATH/$CANAME_LABEL" || { _err "Error copying ca file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            docker cp "$_cfullchain" "$container_id:$DESTINATION_PATH/$FULLCHAINNAME_LABEL" || { _err "Error copying fullchain file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            _info "$CERTTYPE_LABEL certificate type files successfully copied to container: $container_id ($container_name)"
            ...
            docker cp "$_ckey" "$container_id:$DESTINATION_PATH/$KEYNAME_LABEL" || { _err "Error copying key file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            docker cp "$_cfullchain" "$container_id:$DESTINATION_PATH/$FULLCHAINNAME_LABEL" || { _err "Error copying cert file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
            _info "$CERTTYPE_LABEL certificate type files successfully copied to container: $container_id ($container_name)"
            ...
            if ! BUNDLENAME_LABEL=$(docker inspect --format='{{index .Config.Labels "sh.acme.autoload.bundlename"}}' "$container_id"); then
                _err "Error getting sh.acme.autoload.bundlename for $container_id ($container_name)"
                ERROR_FLAG=true
                continue
            fi
			...
			if [ "$?" != "0" ]; then
				_err "Error generating bundled file for $container_id ($container_name)"
                ERROR_FLAG=true
                continue
			fi
            ...
            docker cp "/tmp/bundle.pem" "$container_id:$DESTINATION_PATH/$BUNDLENAME_LABEL" || { _err "Error copying VNC cert file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
			rm /tmp/bundle.pem
           _info "$CERTTYPE_LABEL certificate type files successfully copied to container: $container_id ($container_name)"
            ...
            docker cp "$_cfullchain" "$container_id:$DESTINATION_PATH/$FULLCHAINNAME_LABEL" || { _err "Error copying cert file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
			# Copy the bundled files into the container
            docker cp "/tmp/bundle.pem" "$container_id:$DESTINATION_PATH/$BUNDLENAME_LABEL" || { _err "Error copying bundled file to $container_id ($container_name)"; ERROR_FLAG=true; continue; }
			rm /tmp/bundle.pem
            _info "$CERTTYPE_LABEL certificate type files successfully copied to container: $container_id ($container_name)"
            ...
            ERROR_FLAG=true
            _err "Error, incorrect certificate type defined in sh.acme.autoload.certtype for $container_id ($container_name)"
			continue
        fi
		
		_info "Restarting container: $container_id ($container_name)"
        docker restart "$container_id" || { _err "Error restarting $container_id  ($container_name)"; ERROR_FLAG=true; continue; }
    
	    _info "Container restarted: $container_id ($container_name)"
